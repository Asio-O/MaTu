# 码图

单用户桌面工具：实时读取本地 C# 项目源码，把类型与方法之间的依赖画成图；源码改动后图自动更新，支持在多个项目间切换。

- 外壳：WinUI 3 + WebView2
- 分析：Roslyn（纯语法 L1 + 按需语义 L2）
- 渲染：Cytoscape.js + cytoscape-node-html-label（mermaid 风格 HTML 卡片）

## 现在能做什么

| 能力 | 说明 |
| --- | --- |
| 首屏骨架 | 打开目录后按「命名空间聚合」显示，大项目首屏只有十几个节点 |
| 铺满工作区 | 顶层按工作区的长宽比排列卡片，不在宽窗口里挤成一根竖条；窗口尺寸变了会重铺一次 |
| 逐层展开 | 单击命名空间 / 类型，卡片自己长大成托盘，子节点排在卡片内部（可递归嵌套）；双击空白折叠全部 |
| 整棵展开 | 双击一个容器，把它下面一层层的容器一起展开（一次最多 60 个，防止大项目瞬间铺满） |
| 边跟随展开 | 容器展开后，原来连在它卡片上的聚合边改挂到里面真正参与的子节点上，一路到方法 |
| 拖动 | 拖托盘时里面的子节点整体跟着走，松手后停在拖到的位置；子节点拖不出父容器（贴边停住），`0` 键清掉全部位移 |
| 悬停高亮 | 鼠标停在节点上时，跟它有关系的边点亮、其余的压暗，一眼看清这个节点的依赖 |
| 位置提示 | 顶部面包屑显示当前选中节点在包含树里的位置，点中间一段可以跳回去 |
| 类型级依赖 | 继承/实现、类型级调用聚合边；嵌套类型之间画归属虚线 |
| 精确调用 | 点开某个类型时用语义模型精确解析它的调用，消掉 L1 的同名方法虚边 |
| 跳转源码 | 单击方法节点，用 VS Code / Visual Studio 打开对应文件与行 |
| 自动刷新 | 文件保存后 400ms 防抖重建；删除/新增文件同样触发 |
| 手动重跑 | 工具栏和标题栏都有「重新分析」，不用去动源码也能重建一次 |
| 搜索 | 工具栏搜索框按类型名 / 命名空间 / 方法名过滤，命中路径自动展开 |
| 调用链 | 选中节点后按「调用链」（或右键节点）追踪上下 N 层调用 |
| 导出 | 当前视图导出 PNG / mermaid 文本 / JSON（收在一个菜单里），经宿主「另存为」落盘 |
| 深浅色 | 跟随系统，也可以随时切成浅色 / 深色；宿主标题栏（底色、标题文字、窗口按钮）跟着一起切 |
| 快捷键面板 | 按 `?` 或点右上角问号，列出全部操作 |
| 多项目 | 切换项目时旧项目的监听器、缓存、分析器一律释放 |

## 架构

```
码图.exe                     ← 同一个可执行文件的两种身份
├── 默认         WinUI 界面 + WebView2 宿主
└── --analyzer   stdio 上的分析器进程（JSON 行协议）

Graph/                       分析层（不依赖 WinUI，可被 tools/GraphDump 复用）
├── Models.cs                GraphNode / GraphEdge / GraphSnapshot
├── FileSkeleton.cs          单文件解析结果（L1 磁盘缓存的存储单元）
├── SyntaxFacts.cs           语法层纯函数：类型路径、方法签名、可见性符号
├── SkeletonBuilder.cs       ParseFile → Compose：文件解析 与 图组装 分离
├── SkeletonCache.cs         L1 磁盘缓存：文件哈希 → 解析结果
├── ReverseIndex.cs          名字 → 声明它的文件 / 引用它的文件
├── SemanticResolver.cs      L2 语义精确化 + 精确级联失效
├── IAnalyzer.cs             进程内实现
├── OutOfProcessAnalyzer.cs  进程外实现（子进程 + JSON 行协议）
├── AnalyzerFactory.cs       选择与自动降级
├── AnalyzerServer.cs        --analyzer 模式的服务端
├── SourceWatcher.cs         文件监听 + 防抖
├── SourceNavigator.cs       探测编辑器并跳转到源码位置
├── ProjectContext.cs        单项目生命周期
└── ProjectRegistry.cs       多项目切换

wwwroot/                     渲染层
├── index.html               设计变量 + 工具条/浮层结构 + 卡片样式
├── app.js                   包含树 + 容器内嵌布局 + diff 渲染、主题、搜索、追踪、导出
├── cytoscape.min.js         本地化，避免 CDN 不可达
└── cytoscape-node-html-label.min.js

tools/                       独立诊断与自检工程（不进入主工程编译）
├── GraphDump/               命令行跑一遍分析，打印统计与不变量
└── webprobe/                WebView2 远程调试探针与端到端自检
```

### 三层聚合模型

节点通过 `parentId` 组成一棵包含树：`方法 → 类型 → 外层类型 → 命名空间`。
前端只推导可见性，不重新计算图数据：

```
可见 = 该节点的全部祖先容器都处于展开状态
```

默认一个容器都不展开，所以首屏只剩命名空间聚合节点。切换成「平铺：类型」模式可以退回到传统的类型平铺视图。

### 界面：配色只有一张表，状态只走 data

页面里所有颜色都来自 `app.js` 顶部的 `THEMES`，它同时喂三个地方：

1. 写成 CSS 变量挂到 `<html>` 上（`--surface` / `--ink` / `--shadow-*` …），
   工具条、浮层、卡片样式全部走 `var(--…)`；
2. 给 cytoscape 样式表、卡片模板、导出重绘提供同一份节点色与边色；
3. 渲染图例 —— 图例就是这份表算出来的，不会出现「改了配色、图例还是旧色」。

卡片上那几个派生色（淡底、描边、标题字色）由 `accentVars()` 按主色现算成内联变量，
而不是用 CSS `color-mix()`：那个函数要 WebView2 141+，而这套东西得跑在任何一台
装了 WebView2 的机器上。切主题时重建 cytoscape 样式表并重画一次。

**标题栏（宿主那一半）** 的主题由页面驱动，分三段落：

1. 页面在启动时和每次切换时都推一条 `theme` 消息 —— 启动那次不能省，
   否则「系统浅色 + 上次在页面里选了深色」这种组合下，外壳会一直停在系统主题上；
2. 宿主把结果落到 `RootGrid.RequestedTheme`：标题栏的文字、按钮，以及 `App.xaml`
   里 `ThemeDictionaries` 那两把**标题条底色画刷**都跟着换
   （窗口内容延伸进了标题栏，这一条底是我们自己画的，颜色就取页面的底色）；
3. 最小化 / 最大化 / 关闭是系统画的，不吃元素级 `RequestedTheme`，
   只能按深浅显式给一套配色（`ApplyCaptionButtonColors`）。

宿主改完还会回执一条 `shell-theme`（带上根元素 `ActualTheme` 和标题条最终颜色），
页面挂在 `state.shell` 上 —— 标题栏页面自己看不见，自检只能靠这条回执核对。

交互状态（选中 `sel`、悬停 `hovered`）写进节点的 `data`，而不是往 DOM 上加类：
卡片那一层 DOM 是 `cytoscape-node-html-label` 按 `data` 重建的，
从外面挂上去的类活不过下一次渲染。所以卡片模板读 `data` 出类名，
方法芯片则直接用 `node[isMethod][sel = 1]` 这类选择器。

### 布局：容器内嵌，不是把子节点甩到四周

展开一个容器**不会**把子节点散落到它周围的环上，而是让容器卡片自己长大成「托盘」，
子节点排在托盘里面 —— 跟 mermaid 的 subgraph 一样，「谁属于谁」是看出来的，不用顺着边猜。
可以递归：命名空间的托盘里放类型卡片，类型的托盘里再放方法芯片。

布局是自底向上算的，每次渲染整棵可见树重算一遍：

```
卡片自然尺寸（叶子：标题 + 成员列表；方法：按标签宽度量出来的芯片）
  ↓ 自底向上
容器尺寸 = 标题条 + 内边距×2 + 子节点打包结果
  ↓ 自顶向下
绝对坐标（顶层从固定原点向右下铺，展开一个容器只把它后面的东西推开）
```

两种打包方式：

- **方法芯片**按列排 —— 一列读起来就是一张成员表；超过 22 个才开第二列。
- **卡片**用货架打包，但「一行铺多宽」不拍系数：把每个「一行放几列」的宽度都试一遍，
  挑铺出来最接近目标长宽比的那组。拍系数的话，卡片高度稍有变化就会在
  「挤成一列」和「铺成一条」之间反复横跳。

目标长宽比分两种：

```
顶层（根节点）  →  跟着工作区走：画布宽高比是多少，就铺成什么形状
容器内部        →  尽量接近正方形，免得一个托盘被拉成一条横幅
```

顶层这条是后补的：早先顶层也按「尽量接近正方形」挑，结果宽窗口里所有卡片挤成一根竖条，
左右各留一大片空白，字号还被 fit 压到 17%。现在顶层照画布比例铺，同一个项目从 17% 变成 50%。
窗口尺寸变化会按新比例重铺一次（只重排，不动相机 —— 用户自己挪过的视角不该被一次拉伸夺走）。

尺寸和位置都带过渡动画：容器长大是在 `requestAnimationFrame` 里补间 `data(w/h)`
（`cytoscape-node-html-label` 每帧跟着重排卡片），被挤开的邻居同时平移到新位置。

动画是异步的，所以每轮渲染都带一个**代次号**：延迟起的出生动画、被打断的尺寸补间、
拖动途中重取的跟随名单，统统以「当前这一代」为准 —— 过期的那一份直接作废，
不然它会拿着上一轮布局的坐标把节点钉在那儿。一轮动画播完之后还会做一次**落地校正**：
逐个核对节点是不是真的落在布局位置上，差得多的摆正。正常情况下它什么都不做，
但正是它把「子节点跑到父容器外面、而且再也不回来」这类残留状态堵死了。
它只是**兜底**，真正的缺陷必须在源头修 —— 它曾经把「点开容器时卡片没落位」
盖成了 780ms 之后的一次「瞬移」（卡片突然跳到新位置，把子节点包住）。

### 边会跟着展开走：线改挂到真正参与的子节点上

聚合边是逐级算出来的，画的时候也要逐级换：

```
calls（方法→方法） → typeCalls（类型→类型） → nsCalls（命名空间→命名空间）
inherits（类型→类型） → nsInherits（命名空间→命名空间）
```

全折叠时画聚合边是对的 —— 聚合边就是那一层的摘要。但容器一展开，摘要就该让位：
**原来连在父卡片上的线，要改连到里面真正参与的那个子节点上**。

做法是先把这条聚合边顺着聚合链展开到最细一层，再从聚合边的端点沿着路径往下走，
只在「这一层容器确实展开了、下一层又在画布上」时才往下走一步：

| 情况 | 画成 |
| --- | --- |
| 走到明细里的具体节点 | 线的这一端改挂到它身上 |
| 走不动（没展开 / 被聚焦过滤掉） | 仍然挂在原来的父节点上 |
| 两端都走下去了 | 聚合边整条退场 —— 明细边自己已经在画同一件事 |

展开一个命名空间，线会一路挂到方法上（如果类型也展开着），而不是停在中间那一层。
线的粗细和颜色仍然按**基础边自己的类型**画：换了端点，但看上去还是原来那条线，
一眼能认出「就是它挪过去了」。

节点位置是算出来的，不是 cytoscape 的复合节点，所以拖动托盘时里面的子节点不会自动跟着走 ——
拖动过程中把位移同步给所有可见后代，松手时把「拖离布局位置多远」记在父节点上，
重排时由 `placeBox` 生效（子节点坐标本来就从父节点中心推出来，位移自动继承）。
按 `0` 可以把这些手动位移一起清掉。

### 分析的两条路线

- **L1（纯语法）**：只为首屏服务。并行解析所有 `.cs`，秒级完成，产出类型、成员签名、继承与同名调用。
  同名方法会产生虚边——这是 L1 的固有代价。
- **L2（语义，按需）**：用户点开某个类型时才建立一次 Adhoc Compilation，用 `SemanticModel`
  把调用精确落到具体方法符号上，返回精确的 `calls` / `typeCalls` 边替换该类型的 L1 近似。

### 进程内 / 进程外

`AnalyzerFactory` 按 `MATU_ANALYZER` 环境变量或项目规模选择：

| 取值 | 行为 |
| --- | --- |
| `auto`（默认） | 项目 `.cs` 文件数 ≥ 1500 时用独立进程 |
| `1` / `on` | 总是独立进程 |
| `0` / `off` | 总是进程内 |

独立进程失败会自动降级回进程内，并把原因写进日志——分析器的故障最多让当次分析重做一遍。

### 缓存

```
%LOCALAPPDATA%\码图\cache\<项目根路径哈希>\
    meta.json                 格式版本 / 项目根 / 最后打开时间
    files.json                文件路径 → 内容哈希
    skeletons\<哈希>.json      该内容对应的解析结果
```

键是**内容哈希**而不是路径：改一个文件只会让它自己多出一份新缓存。
`meta.json` 里的格式版本不匹配就整目录清掉重扫，绝不尝试复用半个缓存。

改动文件后 L2 缓存不做整体清空，而是用反向索引只作废受影响的类型
（例如改 `Models.cs` 只作废 22 个类型里的 5 个）。

## 构建与运行

```powershell
# 默认：非打包 + 自带 Windows App Runtime，构建完直接运行
dotnet build 码图.csproj -c Debug -p:Platform=x64
.\bin\x64\Debug\net8.0-windows10.0.19041.0\win-x64\码图.exe

# 需要 MSIX 打包形态时
dotnet build 码图.csproj -c Debug -p:Platform=x64 -p:Unpackaged=false
```

> 之所以默认非打包：纯打包形态的 exe 需要机器上已注册 Windows App Runtime，
> 否则直接双击会以 `REGDB_E_CLASSNOTREG` 退出。自带运行时后 exe 可以直接分发运行。

## 快捷操作

| 操作 | 效果 |
| --- | --- |
| 单击命名空间 / 类型 | 卡片长大成托盘（子节点排到里面）或折叠回去 |
| 双击容器 | 把它下面的容器一层层一起展开（一次最多 60 个） |
| 拖动卡片 | 整块一起走 —— 托盘里的子节点跟着位移，松手后停在拖到的位置；子节点会被夹在父容器内，拖不出去 |
| 悬停节点 | 点亮跟它相连的边，其余压暗 |
| 单击方法 | 在编辑器里跳到该方法的源码位置 |
| 右键节点 | 按当前方向与深度追踪调用链 |
| 顶部面包屑 | 显示选中节点在包含树里的位置，点中间一段跳回去 |
| 双击空白 | 折叠全部并退出聚焦 |
| `/` | 聚焦搜索框 |
| `↑` `↓` `Enter` | 在搜索结果里选择并定位 |
| `t` | 对选中节点追踪调用链 |
| `0` / `f` / `+` / `-` | 适配全部（同时清掉手动拖动出来的位移）/ 适配全部 / 放大 / 缩小 |
| `?` | 打开快捷键面板 |
| `Esc` | 退出聚焦 + 折叠全部 |
| `F12` | 打开开发者工具 |

## 自检

```powershell
# 项目统计与不变量（重复节点 ID、悬空边、悬空 parentId…）
dotnet run --project tools/GraphDump -- <项目根>

# 磁盘缓存冷/热启动对比
dotnet run --project tools/GraphDump -- <项目根> --cache

# 反向索引与级联失效
dotnet run --project tools/GraphDump -- <项目根> --index

# L2 语义解析（全部类型或单个类型）
dotnet run --project tools/GraphDump -- <项目根> --resolve-all
dotnet run --project tools/GraphDump -- <项目根> --resolve 命名空间.类型名

# 编辑器探测
dotnet run --project tools/GraphDump -- <项目根> --editor

# 端到端：分析器子进程冒烟 + 真实 WebView2 里模拟点击 + 文件监听热更新
.\tools\webprobe\run.ps1 -Analyzer proc
```

`run.ps1` 的可选参数：`-Port`、`-Configuration`、`-Analyzer auto|inproc|proc`、`-KeepOpen`。

它依赖两个约定：

- 宿主设置 `MATU_CDP_PORT=<端口>` 后，WebView2 会开远程调试端口；`tools/webprobe` 通过 CDP
  读写页面里的 `window.__codemap` 诊断句柄，从而在不看界面的情况下核对渲染结果。
- 热更新自检会在项目根目录造一个 `__matu_probe_tmp.cs` 再删掉；用临时文件而不是改现有文件，
  是为了在任何一步失败时都不留下半改的源码。

## 踩过的坑

按方案 §8 的格式记一笔，都是这套架构里真实踩到、并且已经用回归断言钉住的：

| 现象 | 根因 | 解法 |
| --- | --- | --- |
| 展开后所有子节点叠在容器上 | cytoscape 的 `position()` 交出的是元素内部那个 position 对象本身，`add()` 又会直接引用传进去的对象。子节点都从「父节点的 `position()`」出生，于是整批共用一个对象——动一个等于动全部 | 传进 `add()` / `animate()` 的坐标一律先复制（`copyPos`） |
| 花括号里全被糊住 / 托盘是空的 | 卡片整层 DOM（`z-index: 10`）盖在 canvas 之上，托盘一旦有底色就把画在 canvas 上的子节点全遮了 | 托盘背景透明，嵌套关系只靠边框和标题表达 |
| 卡片比节点大一圈，节点从边框下露出 | `width/height` 写死不等于 `getBoundingClientRect()` —— 默认 `box-sizing: content-box`，边框要另加 | 给卡片加 `box-sizing: border-box` |
| 卡片底下露出纯黑矩形 | cytoscape 填充背景时忽略 `background-color` 自带的 alpha，只看 `background-opacity`（默认 1），于是 `transparent` 被画成不透明黑 | 显式写 `background-opacity: 0`，不要依赖 `background-color: transparent` |
| 展开后线还连在父卡片上 | 聚合边是后端按层级算好的，端点就是父节点；前端直接画「后端给的边」自然不动 | 渲染前把聚合边顺着链条展开到最细一层，再按当前展开状态把端点往下带（`projectEdges`） |
| 同一对节点之间出现两条线（一粗一细） | 聚合边改挂之后，明细边本来就在画同一件事 | 两端都沉下去时聚合边整条退场 |
| 拖动托盘时里面的子节点原地不动 | 子节点是算好坐标摆进去的独立节点，没走 cytoscape 复合节点那套，cytoscape 不知道它们属于谁 | 拖动中把位移同步给可见后代；松手后把位移记在父节点上，重排时由 `placeBox` 继承 |
| 拖动之后一展开就弹回原位 | 布局每次都是整棵树重算，算出来的位置里没有用户拖出来的那一份 | 位移进 `state.offsets`，`placeBox` 落坐标时叠加；`0` 键清掉 |
| 每个卡片都套了一圈虚线边 | `node[isType]:hover` —— cytoscape 没有 `:hover` 伪类，未知伪类被当成恒真条件匹配所有节点 | 去掉伪类，改用 `mouseover`/`mouseout` 事件加 `.hovered` 类 |
| 卡片盖不满节点，边角露出底色 | HTML 排版高度与按公式估算的节点高度对不上 | 把卡片渲染到离屏容器里实测一次，并按内容签名缓存结果 |
| 改一个文件却把整个 L2 缓存清空 | 失效粒度太粗 | 反向索引只作废「改动文件声明的类型」与「引用它们的类型」 |
| 出现「源码与元数据同名类型」两份 | 引用集里混进了项目自身的输出程序集 | 按项目根目录名排除同名 DLL，同时扫描项目 `bin` 补全第三方依赖 |
| 编译里塞进一堆 CS0009 | `bin` 下的原生 DLL 被当成元数据引用 | 用 `AssemblyName.GetAssemblyName` 过滤掉没有托管元数据的文件 |
| 打包形态的 exe 双击就退 | 机器上没有注册 Windows App Runtime | 默认改为非打包 + 自带运行时 |
| 点一下卡片，卡片却留在了一个没人拖过的位置上 | 记位移时拿的是「当前渲染坐标」，而在布局动画进行中它只是中间帧，减出来就是一截假的位移 | 只有真的 `drag` 过才记位移（`state.drag.moved`） |
| 同一张卡片拖第二次，第一次拖出来的位置没了 | 位移基准里已经含了上一次的位移，第二次算出的是「增量」，把总位移顶掉了 | 布局另外存一份不含位移的纯坐标（`lastPureLayout`），位移一律相对它算 |
| 刚展开就切主题（或点「展开一层」），有几条边永远半透明 | 非动画渲染只处理「新出现的」元素，淡入到一半被打断的那些没人把不透明度拉回来 | 非动画渲染把既有节点 / 边的透明度拉回目标值 |
| 子节点能被拖出托盘 | 位移是相对布局坐标记的，没有任何范围约束 | 拖动时实时夹在父容器内部（`dragBounds` + `clampAxis`），越界就贴边 |
| 宽窗口里所有卡片挤成一根竖条，两边全是空白，字号被压到很小 | 顶层打包也在按「尽量接近正方形」挑宽度，压根没看画布长什么样 | 顶层的目标长宽比改成工作区的长宽比，只有容器内部才用正方形 |
| 子节点跑到父容器外面，而且一直不回去 | 位置被某个**过期的异步写入**改过：延迟起的出生动画拿着上一轮布局的坐标、被打断的尺寸补间把 w/h 写回旧值、或者拖动途中重排之后「跟着走」的名单没更新 —— 改完又没有任何渲染来收尾 | ① 渲染加代次号，过期的出生动画直接作废；② 刷新节点数据时掐掉还在跑的补间；③ 拖动中每轮渲染重取一次跟随名单；④ 每轮动画播完做一次落地校正，把偏离布局的节点摆回去 |
| 点开一个容器后，卡片停在原地、子节点却按新布局散到外面（随后被兜底校正盖成一次「瞬移」） | cytoscape 的事件顺序是 `grab → tap → free`，而「点开它」的那次渲染正发生在 `tap` 里 —— 那时 `state.drag.el` 还指着这个节点，于是它被当成「正在拖动」，本轮跳过了落位 | 「正在拖动」的判据补上 `state.drag.moved`：只有真的拖动过才把位置让给鼠标 |

前六条各自对应 `tools/webprobe/probe.js` 里的一条断言，坏了会被自检直接抓出来：
「卡片节点自身不被绘制」「节点尺寸与卡片实测尺寸一致」「没有节点叠在一起」
「各自持有独立的 position 对象」「子节点都排在容器内部」「展开后容器卡片长大了」。

「边要跟着展开走」和「拖动要带着子节点」也各有一条断言：
前者检查「展开后聚合边改挂到了内部节点上」，同时要求**没有聚合边还挂在已展开的容器卡片上**；
后者用 CDP 发真实鼠标事件（不是直接改 `position`，那样绕过 cytoscape 的抓取路径，
根本测不出这个问题）把托盘拖走，再逐个比对子节点的位移是否与容器一致。

第五阶段钉的是界面与拖动这几条：

| 断言 | 防的是什么 |
| --- | --- |
| 切主题后 DOM 变量、卡片描边、底色变量都变了 | 主题只改了 `<html>` 上的类、卡片还留着旧色 |
| 切主题后宿主回执 `ActualTheme` 与标题条底色都换了一套 | 标题栏没跟着主题走（页面换了色，外壳还是系统那套） |
| 帮助面板能开关、列得出快捷键 | 快捷键面板空着或关不掉 |
| 搜索候选有关键词高亮、方向键能移动 | 键盘操作退化 |
| 悬停压暗无关的边、移开恢复 | 高亮只亮不还原，或把正在淡出的边重新点亮 |
| 动画中点一下不会记出拖动位移 | 上面「点一下卡片却留在别处」那个坑 |
| 子节点被拖不出父容器 | 卡片被拖到托盘外面 |
| 连着拖两次不会丢掉上一次的位移 | 位移被第二次拖动顶掉 |
| 「重新分析」推出新快照、且不会卡在「进行中」 | 页面 → 宿主 → 重跑这条链路断了 |
| 顶层铺出来的长宽比接近工作区的长宽比 | 宽窗口里挤成一根竖条（内容没铺满工作区） |
| 拖动途中重排后，重建出来的子节点仍跟着容器走 | 拖动途中刷新让子节点掉队、松手后落在容器外面 |
| 被写过期位置的节点会被落地校正收回容器内 | 过期的异步写入把节点留在旧位置，又没有人来收尾 |
| 点开容器后它自己立刻落到新布局位置上（真实鼠标、且早于落地校正） | 上面那条：卡片没落位、子节点散到容器外，然后被兜底校正盖成一次「瞬移」 |

## 已知限制

- L1 的同名方法调用是近似结果，展开某个类型后才会被 L2 替换成精确边。
- L2 的引用集是「当前进程的框架程序集 + 应用目录 DLL + 被分析项目 `bin` 下的托管 DLL」，
  不解析工程的 NuGet 依赖图（那是 MSBuildWorkspace 的路线，首屏代价不可接受）。
  项目自身输出程序集会被排除，以免编译里出现「源码与元数据同名类型」两份。
  引用缺失时该类型降级保留 L1 结果；`dotnet run --project tools/GraphDump -- <根> --refs`
  可以直接问「当前引用集够不够」。
- 导出 PNG 是自己用 canvas 重画的静态图：类型卡片由 `cytoscape-node-html-label` 渲染成 DOM，
  不在 cytoscape 画布里，直接 `cy.png()` 会得到没有文字的框。
- 内嵌布局是「整棵树重排」：展开一个容器会把它后面的东西推开，所以位置不是手工定格之后就不再变的。
  腾挪都用动画过渡，不会硬跳。
- 拖动位移记在**被拖的那个节点**上：拖托盘就是整个托盘（连同子节点）一起记，单独拖某个子节点则只记它自己。
  子节点被夹在父容器内，所以拖不出托盘范围；把容器折叠再展开，位移还在原地，`0` 清掉。
- 聚合边改挂依赖聚合链上还找得到明细边。某个类型的 L1 调用边被 L2 精确结果顶掉之后，
  如果 L2 又没找到对应的调用，那条命名空间级聚合边会找不到明细、于是留在父卡片上（宁可留着也不丢线）。
- 暂不支持分析 VB / F# / TypeScript。
- 默认按非打包形态构建：机器上若已注册 Windows App Runtime，用 `-p:Unpackaged=false` 可以回到 MSIX。
